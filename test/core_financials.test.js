...
describe('SEC', function() {
  ...
  describe('EdgarOnline', function() {
    ...
    describe('Core Financials', function() {
      it('Core Financials', function(done) {
        var coreFinancials = new EdgarOnline.coreFinancials(key);
        coreFinancials.yearly(function(err, result) {
          assert.equal(result.method, 'GET');
          assert.equal(result.url, 'http://edgaronline.api.mashery.com/v2/corefinancials');
          done();
        });
      });
    });
